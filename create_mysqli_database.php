<?php
// Script pour créer la base de données MySQL en utilisant MySQLi
...
try {
    ...
    if ($conn->select_db($database)) {
        ...
        throw new Exception("Impossible de sélectionner la base de données: " . $conn->error);
    }
    ...
    foreach ($tables as $sql) {
        if ($conn->query($sql) !== TRUE) {
            $tablesCreated = false;
            throw new Exception("Erreur lors de la création des tables: " . $conn->error);
        }
    }
    ...
    if ($userCount == 0) {
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion de l'administrateur: " . $conn->error);
        }
        ...
        $sql = "INSERT INTO users (username, password, email, name, phone, address, role) 
        ...
        // Ajouter des projets
        $sql = "INSERT INTO projects (user_id, title, description, status, progress, start_date, end_date) 
                VALUES ($clientId, 'Rénovation fenêtres salon', 'Installation de 3 fenêtres en aluminium', 'in_progress', 60, '2023-03-15', '2023-04-30')";
        
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du projet 1: " . $conn->error);
        }
        
        $project1Id = $conn->insert_id;
        
        $sql = "INSERT INTO projects (user_id, title, description, status, progress, start_date, end_date) 
                VALUES ($clientId, 'Porte d''entrée PVC', 'Remplacement de la porte d''entrée', 'completed', 100, '2023-02-10', '2023-02-25')";
        
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du projet 2: " . $conn->error);
        }
        
        $project2Id = $conn->insert_id;
        
        $sql = "INSERT INTO projects (user_id, title, description, status, progress, start_date, end_date) 
                VALUES ($clientId, 'Véranda jardin', 'Installation d''une véranda de 20m²', 'pending', 10, '2023-05-01', '2023-06-15')";
        
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du projet 3: " . $conn->error);
        }
        
        $project3Id = $conn->insert_id;
        
        // Ajouter des devis
        $sql = "INSERT INTO quotes (user_id, project_id, title, description, amount, status) 
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du devis 1: " . $conn->error);
        }
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du devis 2: " . $conn->error);
        }
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du devis 3: " . $conn->error);
        }
        
        // Ajouter des messages
        $sql = "INSERT INTO messages (sender_id, receiver_id, subject, content, is_read) 
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du message 1: " . $conn->error);
        }
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du message 2: " . $conn->error);
        }
        
        // Ajouter des messages admin
        $sql = "INSERT INTO admin_messages (user_id, subject, content, is_read) 
        ...
        if ($conn->query($sql) !== TRUE) {
            throw new Exception("Erreur lors de l'insertion du message admin: " . $conn->error);
        }
